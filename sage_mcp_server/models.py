...
class SageConfig(BaseModel):
    ...
    server: str = Field(
        default="https://es.sagecontinuum.org",
...
class DataType(str, Enum):
    ...
    def iio_types(cls) -> List[str]:
        return [
            cls.IIO_TEMP.value,
            cls.IIO_HUMIDITY.value,
            cls.IIO_PRESSURE.value,
            cls.IIO_RESISTANCE.value,
        ]
...
class SelectorRequirements(BaseModel):
    ...
    custom_selectors: Dict[str, str] = Field(
        default_factory=dict, description="Custom selector requirements"
    )
        ...
        try:
            ...
            return cls(
                ...
                custom_selectors={
                    ...
                    if k not in {"resource.gpu", "resource.camera", "resource.usb"}
                },
            )
        except Exception:
            return cls()
    ...
    def from_string(cls, args_str: str) -> "PluginArguments":
        ...
        try:
            ...
            if not isinstance(args_dict, dict):
                raise ValueError("plugin args JSON must be an object")
        except Exception:
...
class PluginSpec(BaseModel):
    ...
    selector: SelectorRequirements = Field(default_factory=SelectorRequirements)
    ...
    def to_dict(self) -> Dict[str, Any]:
        plugin_spec: Dict[str, Any] = {"image": self.image, "volume": {}}
        if self.args.args_dict:
            plugin_spec["args"] = self.args.to_cli_args()
        ...
        return {"name": self.name, "pluginSpec": plugin_spec}
...
class SageJob(BaseModel):
    ...
    def to_dict(self) -> Dict[str, Any]:
        if self.node_value_format == "true":
            nodes_dict: Dict[str, Any] = {node: True for node in self.nodes}
        else:
        ...
        if self.camera_cmd:
            for plugin in base_dict["plugins"]:
                if plugin["name"] == self.camera_plugin_name:
                    spec = plugin.setdefault("pluginSpec", {})
                    spec.setdefault("args", [])
                    spec["args"].extend(["-c", self.camera_cmd])
        return base_dict
